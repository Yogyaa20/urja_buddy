  ...
  Future<void> _fetchProfiles() async {
    try {
      ...
      if (user != null) {
        ...
        if (doc.exists) {
          final data = doc.data()!;
          final familyName = data['familyName'] ?? 'Family';
          final ownerName = data['fullName'] ?? user.email ?? 'Admin';

          setState(() {
            _familyName = familyName;
          });

          // Owner always comes first with isOwner flag
          ...
          if (data.containsKey('family_members')) {
            final members =
                List<Map<String, dynamic>>.from(data['family_members']);
            for (var member in members) {
              if ((member['name'] ?? '').toString().toLowerCase() !=
                  ownerName.toLowerCase()) {
                profiles.add({...member, 'isOwner': false});
              }
            }
          }
...
class _ProfileCard extends StatefulWidget {
  ...
  const _ProfileCard({required this.name, this.isOwner = false});
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: _isHovering
                          ...
                          : widget.isOwner
                              ? UrjaTheme.primaryGreen.withValues(alpha: 0.5)
                              : Colors.transparent,
                      width: 2,
                    ),
