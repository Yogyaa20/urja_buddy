...
''';
  ...
  Future<Map<String, String>> generateEnergyAudit(
      {double? currentKwh, double? prevKwh}) async {
    try {
      final settingsBox = await Hive.openBox('settings');
      final appliancesBox = await Hive.openBox('appliances');
...
''';
      ...
      final raw = await _callGroqChat(
        systemPrompt: 'You are Urja Buddy, a professional energy analyst. Return ONLY raw JSON text (no markdown fences).',
        userMessage: prompt,
      );
  ...
  Future<String> getTipOfTheDay() async {
    return getAIResponse(
        'Give me a specific Tip of the Day based on my highest consuming appliance. If no appliances, give a general saving tip. Keep it under 20 words.');
  }
