  ...
  Future<void> _register() async {
    if (_fullNameController.text.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill in all fields including State')),
      );
      return;
    }
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _indianStates.map((state) {
        return DropdownMenuItem(
          value: state,
          child: Text(state, style: const TextStyle(color: UrjaTheme.textPrimary)),
        );
      }).toList(),
      onChanged: (val) {
        setState(() {
      ...
      dropdownColor: UrjaTheme.darkBackground,
      decoration: InputDecoration(
        hintText: 'Select State',
        hintStyle: const TextStyle(color: UrjaTheme.textSecondary),
        prefixIcon: const Icon(Icons.location_on_outlined, color: UrjaTheme.textSecondary),
        filled: true,
        fillColor: UrjaTheme.glassBorder.withValues(alpha: 0.3),
        border: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12),
